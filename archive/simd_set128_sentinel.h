/*
 * simd_set128: AVX-512 hash set for 128-bit keys with 16-bit h2 and overflow sentinel
 *
...
static inline uint32_t ss128_match(const uint16_t *meta, uint16_t h2) {
    ...
    return ss128_movemask_epi16(_mm256_cmpeq_epi16(lo, needle),
...
static inline uint32_t ss128_empty(const uint16_t *meta) {
    ...
    __m256i lo = _mm256_load_si256((const __m256i *)meta);
    __m256i hi = _mm256_load_si256((const __m256i *)(meta + 16));
    return ss128_movemask_epi16(_mm256_cmpeq_epi16(lo, z),
    ...
    for (uint32_t g = 0; g < old_ng; g++) {
        ...
        for (int s = 0; s < 31; s++) {
            ...
            for (;;) {
                ...
                base[31] |= ss128_overflow_bit(h.hi);
                gi = (gi + 1) & mask;
            }
        }
    }
